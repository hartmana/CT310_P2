...
//		echo 'Salt: ' . $salt;
    ...
    public function isFriend($userID, $potentialFriendID, $dbh)
    {
        $sqlCheck = 'SELECT * FROM friends WHERE userID=? AND userFriendID=?';

        try
        {
            $statement = $dbh->prepare($sqlCheck);

            $result = $statement->execute(array($userID, $potentialFriendID));

            if (!$result)
            {
                return "Error retrieving friend info from database.";
            } else
            {
        ...
        {
            echo 'Database failure: ' . $pdoE->getMessage();
            return false;
        }
    ...
    public function isPendFriend($userID, $friendID, $dbh)
    {
        $sqlCheck = "SELECT * FROM pendFriends WHERE userID=? AND pendFriendID=?";

        try
        {
            $statement = $dbh->prepare($sqlCheck);

            $result = $statement->execute(array($userID, $friendID));
